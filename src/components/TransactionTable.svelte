<script>
	let { paginatedItems = [], sortBy, currentSortColumn, sortAscending } = $props();
			...
			<tbody>
				{#each paginatedItems as transaction (transaction.id)}
					...
					<tr class="table-row">
						...
						<td class="timestamp-col dimmed-text" data-label="Timestamp"
							>{formatTimestamp(transaction.timestamp)}</td
						>
