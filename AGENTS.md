...
2. **Polished, responsive UI** — mobile-first; see `MOBILE_RESPONSIVENESS_ANALYSIS.md`. Landing page is a centered, full-viewport-panel design with a fixed side dot-rail for section navigation (keyboard paging via arrows/PageUp/PageDown/Home/End moves focus to the target section). The backdrop is `ThreeScene.svelte` — the same WebGL wireframe terrain that backs /login, so both public pages share one scene (it is transparent, so the page ground is `--background-color` from `<body>`). The badge and dashboard panel carry a rotating conic-gradient border glow. There is no in-page pause control: looping motion is disabled via `prefers-reduced-motion` only, which is a known WCAG 2.2.2 gap. (`HOMEPAGE_REDESIGN_PROMPT.md` describes the previous editorial design and is historical.)
